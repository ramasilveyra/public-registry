...
function inlineNodeVisitor(node, parent, props, key) {
  ...
  node[key].traverse({
    MemberExpression(path) {
      ...
      const firstMemberExpression = (0, _getFirstMemberExpression2.default)(path);
...
function inline(props, path, parent, node) {
  ...
  if (iteration && (iteration.currentValuePath.node.name === path.node.name || iteration.indexPath && iteration.indexPath.node.name === path.node.name || iteration.arrayPath && iteration.arrayPath.node.name === path.node.name)) {
    return;
  }
  ...
  const isDefaultTruthy = definition && definition.defaultPath && (0, _isTruthy2.default)(definition.defaultPath);
