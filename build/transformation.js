...
function transformation(oldAst, inputFilePath) {
  ...
  const reactComponentVisitor = {
    IfStatement(path) {
      ...
      const isFnWithoutQuickReturn = t.isBlockStatement(path.parent) && t.isArrowFunctionExpression(path.parentPath.parent);
      if (isFnWithoutQuickReturn) {
        const condition = (0, _ast.createCondition)({ testPath: path.get('test') });
    ...
    JSXExpressionContainer(path) {
      ...
      if (t.isStringLiteral(expressionNode)) {
        const text = (0, _ast.createText)(expressionPath.node.value);
        (0, _addToContext2.default)(context, text);
        return;
      }
    ...
    JSXAttribute(path) {
      ...
      if (name === 'dangerouslySetInnerHTML') {
        ...
        const attribute = (0, _ast.createInterpolationUnescaped)(valuePath);
        (0, _addToContext2.default)(context, attribute);
        return;
      }
      if (!valueNode) {
      ...
      if (t.isJSXElement(expression)) {
        const attribute = (0, _ast.createAttribute)({ name, isNode: true });
        (0, _addToContext2.default)(context, attribute, 'attributes');
