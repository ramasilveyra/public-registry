...
function generateProperty({ name, isBoolean, isString, value, valuePath, isRequired, scope }) {
  ...
  const propertyInterpolated = `${startPropertyBeginning}="${generateInterpolationEscaped(valuePath, scope)}"`;
...
function generateIteration({ iterablePath, params, body, scope }) {
  ...
  const iterationArray = [generateScriptlet(`${iterableCode}.forEach((${paramsCode}) => {`), body, generateScriptlet('})')].filter(Boolean);
