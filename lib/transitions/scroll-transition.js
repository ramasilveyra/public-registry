...
function (_React$Component) {
  ...
  _createClass(ScrollTransition, [{
    ...
    key: "needsLastToFirstFix",
    value: function needsLastToFirstFix(index, targetPosition) {
      var lasSlideIndex = this.props.slideCount - 1;
      var realSlideWidth = this.props.slideWidth + this.props.cellSpacing;
      var slideNeedsHack = index === 0 || index === 1 || index === lasSlideIndex;
      var deltaXNeedsHack = this.props.deltaX <= -realSlideWidth;
      var targetPositionNeedsHack = targetPosition === 0 || targetPosition === realSlideWidth || targetPosition === -realSlideWidth;
      var result = slideNeedsHack && targetPositionNeedsHack && deltaXNeedsHack && typeof memoize[index] !== 'undefined';
      return result;
    }
  }, {
    key: "needsFirstToLastFix",
    value: function needsFirstToLastFix(index, targetPosition) {
      var lasSlideIndex = this.props.slideCount - 1;
      var realSlideWidth = this.props.slideWidth + this.props.cellSpacing;
      var slideNeedsHack = index === 0 || index === lasSlideIndex || index === lasSlideIndex - 1;
      var deltaXNeedsHack = this.props.deltaX > realSlideWidth && realSlideWidth * 2 > this.props.deltaX;
      var targetPositionNeedsHack = targetPosition / index === realSlideWidth || index === 0 && targetPosition / this.props.slideCount === realSlideWidth;
      var result = slideNeedsHack && targetPositionNeedsHack && deltaXNeedsHack && typeof memoize[index] !== 'undefined';
      return result;
    }
    /* eslint-disable complexity */
    ...
    value: function getSlideStyles(index, positionValue) {
      ...
      var mayNeedLastToFirstFix = this.props.currentSlide === 0;
      var mayNeedFirstToLastFix = this.props.currentSlide === this.props.slideCount - 1;
      var needsLastToFirstFix = mayNeedLastToFirstFix && this.needsLastToFirstFix(index, targetPosition);
      var needsFirstToLastFix = mayNeedFirstToLastFix && this.needsFirstToLastFix(index, targetPosition);

      if (this.props.enableWrapAroundHack && (needsLastToFirstFix || needsFirstToLastFix)) {
